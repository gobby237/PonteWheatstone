// plotData_zoom_lines_clipped.C
// Macro ROOT: istogramma dai dati forniti e tabella statistica.
// Le linee verticali vengono disegnate SOLO tra i bin (bordi interni)
// e si fermano all'altezza massima delle colonne.

#include <TCanvas.h>
#include <TH1D.h>
#include <TStyle.h>
#include <TPaveText.h>
#include <TLatex.h>
#include <TFile.h>
#include <TMath.h>
#include <TLine.h>
#include <TApplication.h>

#include <vector>
#include <sstream>
#include <iomanip>
#include <iostream>
#include <algorithm>
#include <string>
#include <TSystem.h>

// Funzione con parametri per personalizzare range X, larghezza bin, massimo Y e dimensioni immagine
// xmin_user:  minimo asse X (-1 per automatico)
// xmax_user:  massimo asse X (-1 per automatico)
// bin_width:  larghezza bin (-1 per 10 bin fissi)
// ymax_user:  massimo asse Y (-1 per automatico)
// img_width:  larghezza canvas/immagine in pixel
// img_height: altezza canvas/immagine in pixel
void plotDataZoominv(double xmin_user  = 1920,
                     double xmax_user  = 1984,
                     double bin_width  = -1.0,
                     double ymax_user  = -1.0,
                     double img_width  = 500,
                     double img_height = 600) {
  gSystem->Load("libstdc++");

  // ===== 1) Dati =====
  std::vector<Double_t> data = {
    1956.9, 1955.4, 1952.6, 1957.3, 1952.8,
    1952.4, 1955.7, 1960.2, 1952.1, 1960.9,
    1956.3, 1962.0, 1952.7, 1953.8, 1949.7
  };

  if (data.empty()) {
    std::cerr << "Nessun dato.\n";
    return;
  }

  // ===== 2) Statistiche base =====
  Double_t minv = *std::min_element(data.begin(), data.end());
  Double_t maxv = *std::max_element(data.begin(), data.end());
  Double_t mean = 0, stddev = 0;
  {
    Double_t sum = 0, sum2 = 0;
    for (auto v : data) { sum += v; sum2 += v * v; }
    mean = sum / data.size();
    Double_t var = (sum2 / data.size()) - mean * mean;
    stddev = (var > 0) ? sqrt(var) : 0.0;
  }

  // ===== 3) Binning e range "allargato" =====
  Int_t nbins = 30;
  const Double_t pad_frac = 0.20;
  const Double_t n_std = 4.0;

  Double_t span = maxv - minv;
  if (span == 0) span = 1.0;
  Double_t pad = pad_frac * span;

  Double_t xmin_data = minv - pad;
  Double_t xmax_data = maxv + pad;
  Double_t xmin_std = mean - n_std * stddev;
  Double_t xmax_std = mean + n_std * stddev;

  Double_t xmin = std::min(xmin_data, xmin_std);
  Double_t xmax = std::max(xmax_data, xmax_std);

  if (stddev == 0.0) {
    xmin = minv - pad;
    xmax = maxv + pad;
  }

  // ===== Override range personalizzato =====
  if (xmin_user != -1.0) xmin = xmin_user;
  if (xmax_user != -1.0) xmax = xmax_user;

  // ===== Calcolo nbins basato su bin_width se specificato =====
  if (bin_width > 0.0) {
    nbins = TMath::Ceil((xmax - xmin) / bin_width);
    if (nbins < 1) nbins = 1;
  }

  TH1D *h = new TH1D("h", "R_{n}^{0,inv};#Omega (ohm);Counts", nbins, xmin, xmax);
  h->SetLineColor(TColor::GetColor("#bc51a1"));
  h->SetLineWidth(3);
  h->SetFillStyle(0);

  for (auto v : data) h->Fill(v);

  Double_t entries = h->GetEntries();
  Double_t underflow = h->GetBinContent(0);
  Double_t overflow = h->GetBinContent(h->GetNbinsX() + 1);
  Double_t sigma_percent = (mean != 0.0) ? (stddev / TMath::Abs(mean) * 100.0) : 0.0;

  // ===== 4) Canvas e disegno =====
  gStyle->SetOptStat(0);
  TCanvas *c = new TCanvas("c", "c", (Int_t)img_width, (Int_t)img_height);
  c->SetLeftMargin(0.12);
  c->SetRightMargin(0.05);

  // Applica il range visibile sull'asse X se definito
  if (xmin_user != -1.0 && xmax_user != -1.0) {
    h->GetXaxis()->SetRangeUser(xmin_user, xmax_user);
  }

  // Applica il massimo personalizzato sull'asse Y se definito
  if (ymax_user != -1.0) {
    h->SetMaximum(ymax_user);
  }

  // Nasconde la prima etichetta degli assi X e Y
  h->GetXaxis()->ChangeLabel(1, -1, 0);
  h->GetYaxis()->ChangeLabel(1, -1, 0);

  h->Draw("HIST");

  // Aggiorna il canvas per mostrare i disegni
  c->Update();
  c->Modified();

  // ===== Salvataggio =====
  c->SaveAs("istoRninv.png");
  TFile fout("istoRninv.root", "RECREATE");
  h->Write();
  fout.Close();
  std::cout << "Salvato istoRninv.png e istoRninv.root ("
            << (Int_t)img_width << "x" << (Int_t)img_height << " px)." << std::endl;

  // ===== 5) Gestione Interattiva =====
  if (!gROOT->IsBatch()) {
    std::cout << "Grafico generato. Chiudi la finestra della Canvas per terminare." << std::endl;
    c->WaitPrimitive();
  }
}

#ifndef __CINT__
int main(int argc, char **argv) {
  TApplication theApp("App", &argc, argv);

  // ===== Parametri personalizzabili =====
  // - xmin_user:  minimo asse X visualizzato (-1 per automatico)
  // - xmax_user:  massimo asse X visualizzato (-1 per automatico)
  // - bin_width:  larghezza di ogni bin (-1 per 10 bin fissi)
  // - ymax_user:  massimo asse Y visualizzato (-1 per automatico)
  // - img_width:  larghezza immagine salvata in pixel
  // - img_height: altezza immagine salvata in pixel
  double xmin_user  = 1920;   // Es: 1940.0
  double xmax_user  = 1984;   // Es: 1965.0
  double bin_width  = -1.0;   // Es: 2.0
  double ymax_user  = -1.0;   // Es: 10.0
  double img_width  = 600;   // Es: 1200
  double img_height = 800;    // Es: 800

  plotDataZoominv(xmin_user, xmax_user, bin_width, ymax_user, img_width, img_height);
  theApp.Run();
  return 0;
}
#endif